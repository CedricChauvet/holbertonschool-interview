...
def count_words(subreddit, word_list):
    ...
    if 'data' in data and 'children' in data['data']:
        # Extract titles from all articles in the response and convert
        # them to a list
        titres = list(map(lambda article: article.get('data', {}).
                          get('title', ''), data['data']['children']))

        # Combine all titles into a single string and convert to
        # lowercase  case-insensitive matching
        result = " ".join(titres).lower()

        # Sort and convert the word list to lowercase case-insensitive
        # matching
