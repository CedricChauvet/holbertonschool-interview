...
def count_words(subreddit, word_list, after=None, result="", word_counts=None):
    ...
    if after is not None or result == "":
        ...
        try:
            ...
            data = response.json()
            n_titles = list(map(lambda a: a['data']['title'],
                                data['data']['children']))
            result += ' ' + reduce(lambda x, y: x + ' ' + y, n_titles).lower()
    ...
    if word_list:
        ...
        return count_words(subreddit, word_list[1:],
                           after, result, word_counts)

    # Traitement de la page suivante
    if after:
        return count_words(subreddit, list(word_counts.keys()),
                           after, result, word_counts)

    # Affichage des résultats
    sorted_counts = sorted(word_counts.items(), key=lambda x: (-x[1], x[0]))
    sorted_counts = sorted(filter(lambda x: x[1] > 0,
                           word_counts.items()),
                           key=lambda x: (-x[1], x[0]))
    if sorted_counts == []:
        return
    print("sorted_counts: ", sorted_counts)
    list(map(lambda item: print(f"{item[0]}: {item[1]}"),
             sorted(filter(lambda x: x[1] >= 0, sorted_counts),
             key=lambda x: x[0])))
